...
const FILTER_SELECT_CLASS =
    'w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-[#0F1F2E] outline-none transition focus:border-[#27527A] focus:ring-2 focus:ring-[#D8E4ED]/60';
...
export default function MapDashboard() {
    ...
    const allReports = useMemo(() => {
        return dataMode === 'database' && normalizedDatabaseReports.length > 0
            ? normalizedDatabaseReports
            : fallbackReports;
    }, [dataMode, normalizedDatabaseReports, fallbackReports]);
                    ...
                    <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                        <div>
                            <div className="flex flex-wrap items-center gap-2">
                                <span className="inline-flex items-center gap-1.5 rounded-full bg-[#EFF4F8] px-3 py-1 text-xs font-semibold text-[#D95F5F] ring-1 ring-[#D8E4ED]/35">
                                ...
                                <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 dark:bg-[#17324A] px-3 py-1 text-xs font-semibold text-slate-600 dark:text-slate-200">
                                    ...
                                    {dataMode === 'database' ? 'Data database' : 'Data contoh'}
                                </span>
                            </div>
                            ...
                            <p className="mt-1 max-w-3xl text-sm font-semibold text-[#1A3348] dark:text-slate-200">
                                Gunakan peta untuk melihat titik kejadian, memilih layer, memperbesar area, dan memfilter data berdasarkan kategori, status, wilayah, atau waktu kejadian.
                            </p>

                            <div className="mt-3 rounded-2xl border border-[#D8E4ED] bg-[#EFF4F8] p-3 text-[#0F1F2E] shadow-sm dark:border-white/10 dark:bg-white/5 dark:text-white">
                                <div className="flex items-center gap-2 text-sm font-black">
                                    <MousePointerClick className="h-4 w-4 text-[#27527A] dark:text-[#5BAE8A]" />
                                    Panduan Penggunaan Peta
                                </div>
                                <ol className="mt-2 grid gap-1.5 text-xs font-semibold leading-relaxed md:grid-cols-2">
                                    ...
                                    <li>3. Aktifkan/nonaktifkan layer titik laporan, batas kecamatan, dan KDE.</li>
                                    <li>4. Perbesar/perkecil peta memakai tombol zoom.</li>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            <button
                                ...
                                className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition ${showKDE
                            ...
                            <button
                                ...
                                className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition ${showIncidentPoints
                                        ? 'bg-[#D8E4ED] text-[#D95F5F] ring-1 ring-[#D8E4ED]'
                                        : 'bg-white dark:bg-[#102538] text-slate-600 dark:text-slate-200 ring-1 ring-slate-200 hover:bg-slate-50 dark:bg-[#17324A]'
                                    }`}
                            >
                ...
                <div className="grid min-h-0 flex-1 grid-cols-1 xl:grid-cols-[300px_minmax(0,1fr)_320px]">
                    <aside className="min-h-0 overflow-y-auto border-r border-slate-200 dark:border-white/10 bg-white dark:bg-[#102538] p-4">
                        <div className="mb-4 flex items-center gap-2">
                            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#EFF4F8] text-[#D95F5F]">
                        ...
                        <div className="space-y-4">
                            ...
                            <button
                                ...
                                className="w-full rounded-xl border border-[#D8E4ED] bg-white px-3 py-2 text-sm font-black text-[#1A3348] transition hover:bg-[#EFF4F8] dark:border-white/10 dark:bg-[#102538] dark:text-white dark:hover:bg-[#17324A]"
                            >
                                Reset Filter
                            </button>
                            ...
                            <div className="rounded-2xl bg-slate-900 p-4 text-white">
                                ...
                                <p className="mt-1 text-xs leading-relaxed text-slate-300">
                                    {showIncidentPoints
                                        ...
                                        : 'Layer Titik Laporan sedang nonaktif, marker disembunyikan dari peta.'}
                                </p>

                                <p className="mt-2 text-[11px] leading-relaxed text-slate-400">
                                    {dataMode === 'database'
                                        ? 'Menggunakan laporan yang sudah masuk database.'
                                        : 'Database belum tersedia, memakai data contoh untuk tampilan peta.'}
                                </p>
                            </div>

                            {error && (
                                <div className="rounded-2xl border border-[#D8E4ED] bg-[#EFF4F8] p-3 text-sm text-[#1A3348]">
                                    {error}
                                </div>
                            )}
                        </div>
                    ...
                    <main className="relative min-h-0 overflow-hidden bg-slate-100 dark:bg-[#17324A]">
                        <div className="pointer-events-none absolute left-4 top-4 z-[520] rounded-2xl border border-[#D8E4ED] bg-white/95 px-4 py-2 text-xs font-black text-[#0F1F2E] shadow-xl backdrop-blur">
                            {showIncidentPoints
                                ? `Menampilkan ${formatNumber(filteredReports.length)} kejadian sesuai filter.`
                                : 'Layer Titik Laporan nonaktif.'}
                        </div>
                        ...
                        <MapView
                            ...
                            hotspotClusters={!showKDE ? kdeZones as any : []}
                        ...
                        {!loading && mapReports.length === 0 && (
                            <div className="pointer-events-none absolute inset-x-4 top-4 z-[500] rounded-2xl border border-slate-200 bg-white/95 p-4 text-sm font-bold text-[#1A3348] shadow-xl backdrop-blur dark:border-white/10 dark:bg-[#102538]/95 dark:text-white">
                                {emptyMapMessage}
                            </div>
                        )}
                    </main>
                    ...
                    {showAnalysisPanel && (
                        <aside className="min-h-0 overflow-y-auto border-l border-slate-200 dark:border-white/10 bg-white dark:bg-[#102538] p-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="rounded-2xl bg-[#EFF4F8] p-3 ring-1 ring-[#D8E4ED]/35">
                                    <p className="text-xs font-semibold text-[#D95F5F]">
                                        Total Titik
                                    </p>
                                    <p className="mt-1 text-2xl font-black text-[#1A3348]">
                                ...
                                <div className="col-span-2 rounded-2xl bg-[#EFF4F8] p-3 ring-1 ring-[#D8E4ED]">
                                    <p className="text-xs font-semibold text-[#D95F5F]">
                                        Kategori Dominan
                                    </p>
                                    <p className="mt-1 text-sm font-bold text-[#0F1F2E]">
                                        {dominantCategory}
                                    </p>
                                </div>
                                            ...
                                            return (
                                                <div key={item.name}>
                                                    ...
                                                    <div className="h-2 overflow-hidden rounded-full bg-slate-100 dark:bg-[#17324A]">
                                                        <div
                                                            className="h-full rounded-full bg-[#27527A]"
