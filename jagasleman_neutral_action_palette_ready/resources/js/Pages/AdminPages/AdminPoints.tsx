...
function openPointsPdf(reports: IncidentReport[]) {
    ...
    const html = `<!doctype html><html><head><title>Data Titik Terverifikasi JagaSleman</title><style>
        body{font-family:Arial,sans-serif;margin:28px;color:#0F1F2E}h1{margin:0;font-size:24px}.muted{color:#64748b;font-size:12px}.header{border-bottom:3px solid #D95F5F;padding-bottom:14px;margin-bottom:18px}.summary{display:flex;gap:10px;margin:16px 0}.card{border:1px solid #D8E4ED;border-radius:14px;padding:12px;background:#f8fafc}.card b{font-size:22px}table{width:100%;border-collapse:collapse}th{background:#0F1F2E;color:#fff;text-align:left;font-size:11px;padding:10px}td{border-bottom:1px solid #e2e8f0;font-size:11px;padding:9px;vertical-align:top}button{padding:10px 16px;border:0;border-radius:12px;background:#D95F5F;font-weight:700;cursor:pointer;float:right}@media print{button{display:none}}
    </style></head><body>
    ...
    return (
        <AdminLayout title="Titik Tervalidasi">
            <div className="space-y-6">
                    ...
                    <div className="grid gap-5r p-6 text-white lg:grid-cols-[1.15fr_.85fr] lg:p-8">
                        <div>
                            ...
                            <h1 className="mt-4 text-3xl font-black tracking-tight md:text-4xl">
                                Edit dan hapus titik laporan yang sudah disetujui.
                            </h1>
                            <p className="mt-3 max-w-3xl text-sm leading-relaxed text-white/70">
                                Halaman ini fokus pada data approved yang sudah menjadi titik WebGIS. Admin dapat memperbaiki atribut, mengoreksi koordinat, melihat foto bukti, menghapus data, dan export PDF.
                            </p>
                            <div className="mt-5 flex flex-wrap gap-3">
                                <button type="button" onClick={loadReports} className="rounded-2xl bg-white dark:bg-[#102538] px-5 py-3 text-sm font-black text-[#0F1F2E] shadow-lg transition hover:bg-[#EFF4F8]">
                ...
                <section className="rounded-[2rem] border border-[#D8E4ED] bg-white dark:bg-[#102538] p-5 shadow-xl shadow-slate-200/60 dark:border-white/10 dark:bg-white dark:bg-[#102538]/5">
                    <div className="grid gap-3 lg:grid-cols-[1fr_220px_220px_auto]">
                        ...
                        <div>
                            ...
                            <select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)} className="h-12 w-full rounded-2xl border border-[#D8E4ED] bg-[#F8FAFC] px-4 text-sm font-black outline-none transition focus:border-[#D95F5F] focus:bg-white dark:bg-[#102538] focus:ring-4 focus:ring-[#D8E4ED]/35 dark:border-white/10 dark:bg-white dark:bg-[#102538]/5">
                        ...
                        <div>
                            ...
                            <select value={districtFilter} onChange={(event) => setDistrictFilter(event.target.value)} className="h-12 w-full rounded-2xl border border-[#D8E4ED] bg-[#F8FAFC] px-4 text-sm font-black outline-none transition focus:border-[#D95F5F] focus:bg-white dark:bg-[#102538] focus:ring-4 focus:ring-[#D8E4ED]/35 dark:border-white/10 dark:bg-white dark:bg-[#102538]/5">
                                <option value="all">Semua Kecamatan</option>
                                {districtOptions.map((item) => <option key={item} value={item}>{item}</option>)}
                            </select>
                        </div>
                        <div className="flex items-end">
                            <button type="button" onClick={resetFilter} className="h-12 rounded-2xl border border-[#D8E4ED] bg-white dark:bg-[#102538] px-5 text-sm font-black text-slate-700 dark:text-slate-100 transition hover:bg-[#EFF4F8] dark:border-white/10 dark:bg-white dark:bg-[#102538]/5 dark:text-white">Reset</button>
                        </div>
                    </div>

                    {error && <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-bold text-red-700">{error}</div>}

                    <div className="mt-5 overflow-hidden rounded-3xl border border-[#D8E4ED] dark:border-white/10">
                            ...
                            <table className="min-w-[1120px] w-full divide-y divide-slate-200 dark:divide-white/10">
                                ...
                                <tbody className="divide-y divide-slate-100 bg-white dark:bg-[#102538] dark:divide-white/10 dark:bg-transparent">
                                    {loading ? (
                                        ...
                                        return (
                                            <tr key={report.id} className="transition hover:bg-[#EFF4F8]/50 dark:hover:bg-white dark:bg-[#102538]/5">
                                                <td className="px-5 py-4 align-top">
                                                    {photos[0] ? (
                                                        <button type="button" onClick={() => setSelectedReport(report)} className="relative block">
                                                            ...
                                                            {photos.length > 1 && <span className="absolute -right-2 -top-2 rounded-full bg-[#0F1F2E] px-2 py-0.5 text-[10px] font-black text-white">+{photos.length - 1}</span>}
                                                        </button>
                                                    ) : (
                                                ...
                                                <td className="px-5 py-4 align-top">
                                                    <div className="text-xs font-black text-[#D95F5F]">{getReportCode(report)}</div>
                                                    <div className="mt-1 max-w-xs text-sm font-black text-[#0F1F2E] dark:text-white">{report.incident_type || '-'}</div>
                                                    <div className="mt-2 inline-flex rounded-full border border-[#D8E4ED] bg-[#EFF4F8] px-2.5 py-1 text-[11px] font-black text-[#D95F5F]">Terverifikasi</div>
                                                </td>
                                                <td className="px-5 py-4 align-top">
                                                ...
                                                <td className="px-5 py-4 align-top">
                                                    <div className="flex justify-end gap-2">
                                                        ...
                                                        <button type="button" onClick={() => openEdit(report)} className="rounded-xl bg-[#D95F5F] px-3 py-2 text-xs font-black text-white transition hover:bg-[#0F1F2E]"><Edit3 className="mr-1 inline h-3.5 w-3.5" />Edit</button>
                    ...
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
                        <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-[2rem] bg-white dark:bg-[#102538] p-6 shadow-2xl dark:bg-[#122334]">
                            <div className="flex items-start justify-between gap-4">
                                <div>
                                    <p className="text-xs font-black uppercase tracking-[0.22em] text-[#D95F5F]">Detail Titik Tervalidasi</p>
                            ...
                            <div className="mt-6 flex flex-wrap justify-end gap-2">
                                <button type="button" onClick={() => openEdit(selectedReport)} className="rounded-2xl bg-[#D95F5F] px-4 py-2 text-sm font-black text-white"><Edit3 className="mr-2 inline h-4 w-4" />Edit</button>
                ...
                {editingReport && editForm && (
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
                        <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-[2rem] bg-white dark:bg-[#102538] p-6 shadow-2xl dark:bg-[#122334]">
                            <div className="flex items-start justify-between gap-4">
                                <div>
                                    <p className="text-xs font-black uppercase tracking-[0.22em] text-[#D95F5F]">Edit Titik Tervalidasi</p>
                                    <h2 className="mt-2 text-2xl font-black text-[#0F1F2E] dark:text-white">{getReportCode(editingReport)}</h2>
                                </div>
                            ...
                            <div className="mt-6 grid gap-4 md:grid-cols-2">
                                <div>
                                    ...
                                    <input value={editForm.title} onChange={(event) => updateForm('title', event.target.value)} className="h-12 w-full rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#17324A] px-4 text-sm font-semibold outline-none focus:border-[#D95F5F] focus:ring-4 focus:ring-[#D8E4ED]/35" />
                                </div>
                                <div>
                                    ...
                                    <select value={editForm.incident_type} onChange={(event) => updateForm('incident_type', event.target.value)} className="h-12 w-full rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#17324A] px-4 text-sm font-semibold outline-none focus:border-[#D95F5F] focus:ring-4 focus:ring-[#D8E4ED]/35">
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <input value={editForm.location} onChange={(event) => updateForm('location', event.target.value)} className="h-12 w-full rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#17324A] px-4 text-sm font-semibold outline-none focus:border-[#D95F5F] focus:ring-4 focus:ring-[#D8E4ED]/35" />
                                </div>
                                <div>
                                    ...
                                    <input value={editForm.district} onChange={(event) => updateForm('district', event.target.value)} className="h-12 w-full rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#17324A] px-4 text-sm font-semibold outline-none focus:border-[#D95F5F] focus:ring-4 focus:ring-[#D8E4ED]/35" />
                                </div>
                                <div>
                                    ...
                                    <input value={editForm.village} onChange={(event) => updateForm('village', event.target.value)} className="h-12 w-full rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#17324A] px-4 text-sm font-semibold outline-none focus:border-[#D95F5F] focus:ring-4 focus:ring-[#D8E4ED]/35" />
                                </div>
                                <div>
                                    ...
                                    <input value={editForm.latitude} onChange={(event) => updateForm('latitude', event.target.value)} className="h-12 w-full rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#17324A] px-4 text-sm font-semibold outline-none focus:border-[#D95F5F] focus:ring-4 focus:ring-[#D8E4ED]/35" />
                                </div>
                                <div>
                                    ...
                                    <input value={editForm.longitude} onChange={(event) => updateForm('longitude', event.target.value)} className="h-12 w-full rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#17324A] px-4 text-sm font-semibold outline-none focus:border-[#D95F5F] focus:ring-4 focus:ring-[#D8E4ED]/35" />
                                </div>
                                <div>
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <textarea value={editForm.description} onChange={(event) => updateForm('description', event.target.value)} rows={5} className="w-full rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#17324A] px-4 py-3 text-sm font-semibold outline-none focus:border-[#D95F5F] focus:ring-4 focus:ring-[#D8E4ED]/35" />
                                </div>
                                <div className="md:col-span-2">
                            ...
                            <div className="mt-6 flex flex-wrap justify-end gap-2">
                                ...
                                <button type="button" disabled={saving} onClick={saveEdit} className="rounded-2xl bg-[#D95F5F] px-5 py-3 text-sm font-black text-white disabled:opacity-60">
