...
export function getHotspotColor(
  ...
  if (type === "hotspot") {
    ...
    if (zScore > 2.5) return "#D95F5F"; // Red
    if (zScore > 2) return "#D95F5F"; // Light red
    return "#D95F5F"; // Very light red
  } else if (type === "coldspot") {
    // Blue gradient
    if (zScore < -3) return "#1e3a8a"; // Dark blue
    if (zScore < -2.5) return "#D95F5F"; // Blue
...
export const HOTSPOT_LEGEND = [
  ...
  { color: "#D95F5F", label: "Hotspot Tinggi (Z > 2.5)", type: "hotspot" },
  { color: "#D95F5F", label: "Hotspot Sedang (Z > 2)", type: "hotspot" },
  { color: "#D95F5F", label: "Hotspot Rendah (1.96 < Z < 2)", type: "hotspot" },
  { color: "#F59E0B", label: "Normal/Neutral", type: "neutral" },
  { color: "#93c5fd", label: "Coldspot Rendah (-2 < Z < -1.96)", type: "coldspot" },
  { color: "#60a5fa", label: "Coldspot Sedang (Z < -2)", type: "coldspot" },
  { color: "#D95F5F", label: "Coldspot Tinggi (Z < -2.5)", type: "coldspot" },
